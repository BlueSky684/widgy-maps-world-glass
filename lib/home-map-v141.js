...
// v141 uses a 2x supersampled emissive atlas rebuilt from the untouched NASA source.
// Urban network, city body and compact core were created at 3654x1722 and area-downsampled.
let textures;
...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    const networkRaw=gunzipSync(readFileSync(asset('night-network-v141.bin.gz')));
    const bodyRaw=gunzipSync(readFileSync(asset('night-body-v141.bin.gz')));
    const coreRaw=gunzipSync(readFileSync(asset('night-core-v141.bin.gz')));
    const expected=WIDTH*HEIGHT*2;
    if(networkRaw.length!==expected||bodyRaw.length!==expected||coreRaw.length!==expected)
      throw Error('Invalid v141 night layer');
    const networkTone=new Float32Array(WIDTH*HEIGHT);
    const bodyTone=new Float32Array(WIDTH*HEIGHT);
    const coreTone=new Float32Array(WIDTH*HEIGHT);
    for(let p=0;p<networkTone.length;p++) {
      const n=networkRaw.readUInt16LE(p*2)/384;
      const b=bodyRaw.readUInt16LE(p*2)/384;
      const c=coreRaw.readUInt16LE(p*2)/384;
      // Separate global responses let weak networks, city mass and strong cores
      // keep different contrast without regional tuning or highlight overshoot.
      networkTone[p]=n<.06?0:138*Math.pow(n/(n+2.4),.72);
      bodyTone[p]=b<=0?0:238*Math.pow(b/(b+3.8),.62);
      coreTone[p]=c<=0?0:210*Math.pow(c/(c+8.5),.80);
    }
    return {base,networkTone,bodyTone,coreTone,
      geo:coordinateNormals('reference-coordinates-v127.bin.gz')};
...
// v141 high-res luxury palette. The supersampled atlas provides continuity;
// runtime colour stays clean yellow-gold with a restrained ivory centre.
// There is no shine/starburst layer or directional kernel.
const NETWORK=[1,.91,.55],BODY=[1,.87,.40],IVORY=[1,.98,.84];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  const {base,networkTone,bodyTone,coreTone,geo}=await getTextures();
  const out=Buffer.alloc(WIDTH*HEIGHT*3);
  const networkSignal=Buffer.alloc(WIDTH*HEIGHT);
  const bodySignal=Buffer.alloc(WIDTH*HEIGHT);
  const coreSignal=Buffer.alloc(WIDTH*HEIGHT);
  ...
  for(let p=0;p<darkness.length;p++) {
    ...
    const night=darkness[p];
    networkSignal[p]=Math.round(Math.min(135,networkTone[p]*night));
    bodySignal[p]=Math.round(Math.min(195,bodyTone[p]*night));
    coreSignal[p]=Math.round(Math.min(160,coreTone[p]*night));
  }
  // Only CITY BODY receives isotropic glow. Network remains crisp and core
  // remains same-pixel/round, so no cross/star geometry can be introduced.
  const [bodyNear,bodyWide]=await Promise.all([.58,1.35].map(sigma=>
    sharp(bodySignal,{raw:{width:WIDTH,height:HEIGHT,channels:1}})
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    // v137: decorative blue atmospheric arc intentionally removed. The geographic
    // UTC solar day/night shading remains unchanged and continues to drive terrain
    // and city-light visibility.
    const veil=1-.08*smooth(.4,.94,y/HEIGHT)*(1-smooth(.25,.55,x/WIDTH));
    for(let k=0;k<3;k++) {
      const terrain=([1,3,5][k]*(1-land)+base[i+k]*land)*gain;
      const backdrop=clamp(terrain*veil,0,255);
      // Three independent layers:
      // 1) crisp urban network (direct, no blur)
      // 2) round city body glow (isotropic Gaussian only)
      // 3) compact ivory core (direct, no blur)
      const network=networkSignal[p]*.24*NETWORK[k];
      const bodyPoint=bodySignal[p]*.24*BODY[k];
      const cityBody=bodyNear[p]*.52*BODY[k]*night;
      const cityHalo=bodyWide[p]*.075*BODY[k]*night;
      const ivoryCore=coreSignal[p]*.38*IVORY[k];
      const transmission=(1-network/255)*(1-bodyPoint/255)*(1-cityBody/255)*(1-cityHalo/255)*(1-ivoryCore/255);
