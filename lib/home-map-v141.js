...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    for(let p=0;p<networkTone.length;p++) {
      ...
      networkTone[p]=n<.06?0:126*Math.pow(n/(n+2.6),.74);
      bodyTone[p]=b<=0?0:246*Math.pow(b/(b+3.3),.60);
      coreTone[p]=c<=0?0:218*Math.pow(c/(c+7.8),.80);
    }
...
const NETWORK=[1,.87,.30],BODY=[1,.83,.20],IVORY=[1,.97,.72];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let p=0;p<darkness.length;p++) {
    ...
    networkSignal[p]=Math.round(Math.min(120,networkTone[p]*night));
    bodySignal[p]=Math.round(Math.min(205,bodyTone[p]*night));
    coreSignal[p]=Math.round(Math.min(165,coreTone[p]*night));
  }
  ...
  const [bodyNear,bodyWide]=await Promise.all([.92,2.20].map(sigma=>
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const network=networkSignal[p]*.12*NETWORK[k];
      const bodyPoint=bodySignal[p]*.18*BODY[k];
      const cityBody=bodyNear[p]*.72*BODY[k]*night;
      const cityHalo=bodyWide[p]*.13*BODY[k]*night;
      const ivoryCore=coreSignal[p]*.34*IVORY[k];
