...
const BODY=[1,.88,.45],IVORY=[1,.98,.80],NEAR=[1,.80,.28],FAR=[1,.70,.22];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  const [near,far]=await Promise.all([.54,1.28].map(sigma=>
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const ivory=clamp((nightSignal[p]-120)*.12,0,15)*IVORY[k];
      const local=near[p]*.19*NEAR[k]*night;
      const wide=far[p]*.028*FAR[k]*night;
