...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    for(let p=0;p<cores.length;p++) {
      ...
      cores[p]=244*Math.pow(energy/(energy+6.7),.72);
    }
...
const BODY=[1,.90,.66],IVORY=[1,.97,.86],NEAR=[1,.82,.46],FAR=[1,.72,.35];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let p=0;p<darkness.length;p++) {
    ...
    glowSignal[p]=Math.round(Math.min(compact,160));
  }
  const [near,far]=await Promise.all([.50,1.30].map(sigma=>
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const ivory=clamp((nightSignal[p]-118)*.16,0,18)*IVORY[k];
      const local=near[p]*.14*NEAR[k]*night;
      const wide=far[p]*.018*FAR[k]*night;
