...
// v139 uses a completely new Luxury Yellow-Gold scalar master rebuilt from the untouched
// NASA source. No previous generated night signal is consumed.
let textures;
...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    const signal=gunzipSync(readFileSync(asset('night-signal-v139.bin.gz')));
    if(signal.length!==WIDTH*HEIGHT*2)throw Error('Invalid v139 night signal');
    const cores=new Float32Array(WIDTH*HEIGHT);
    for(let p=0;p<cores.length;p++) {
      const energy=signal.readUInt16LE(p*2)/384;
      // v137 approved soft-knee: prioritize crisp network separation and compact
      // points while retaining a realistic hierarchy between weak and dense areas.
      // One global response only; no regional or continent-specific gain.
      cores[p]=246*Math.pow(energy/(energy+6.4),.70);
    }
...
// v139 technical candidate: Luxury Yellow-Gold Clean Rebuild.
// Fresh v139 scalar master + bright clean yellow-gold body + subtle ivory core.
// No shine or directional flare effect: all strong cores remain compact and
// round, with only restrained isotropic micro-glow.
const BODY=[1,.90,.46],IVORY=[1,.99,.82],NEAR=[1,.84,.35],FAR=[1,.74,.26];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  const {base,cores,geo}=await getTextures();
  const out=Buffer.alloc(WIDTH*HEIGHT*3),nightSignal=Buffer.alloc(WIDTH*HEIGHT),glowSignal=Buffer.alloc(WIDTH*HEIGHT);
  ...
  for(let p=0;p<darkness.length;p++) {
    ...
    const raw=cores[p]*darkness[p];
    // Preserve weak/mid network detail unchanged. Above 148, compress smoothly
    // toward ~202 instead of allowing a white-hot spike. This is global and
    // shape-neutral: no radial rays, flare kernels or directional sharpening.
    const compact=raw<=154?raw:154+46*(1-Math.exp(-(raw-154)/52));
    nightSignal[p]=Math.round(compact);
    // Glow gets a lower ceiling than the point core so dense cities stay compact.
    glowSignal[p]=Math.round(Math.min(compact,165));
  }
  const [near,far]=await Promise.all([.62,1.45].map(sigma=>
    sharp(glowSignal,{raw:{width:WIDTH,height:HEIGHT,channels:1}})
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const body=nightSignal[p]*BODY[k];
      // Same-pixel ivory lift only. It adds a clean premium core without
      // any blur or directional kernel.
      const ivory=clamp((nightSignal[p]-125)*.10,0,12)*IVORY[k];
      const local=near[p]*.24*NEAR[k]*night;
      const wide=far[p]*.035*FAR[k]*night;
      const transmission=(1-body/255)*(1-ivory/255)*(1-local/255)*(1-wide/255);
