...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let y=0;y<HEIGHT;y++){
    ...
    for(let x=0;x<WIDTH;x++){
      ...
      // First remove the warm emissive component on the daylight side.
      // Night pixels remain byte-for-byte equal to the approved master.
      const attenuation=1-.92*day*w;
      const r0=R*attenuation,g0=G*attenuation,b0=B*attenuation;

      // Then reveal a restrained cool daylight terrain treatment. Oceans stay
      // near-black; blue/grey land gets a visible lift so the solar terminator
      // is obvious in Widgy without repainting the approved night side.
      const luma=.24*R+.54*G+.22*B;
      const cool=clamp((B-R+34)/70,0,1);
      const land=clamp((luma-7)/42,0,1)*(.58+.42*cool)*(1-.72*w);
      const lift=day*land;
      out[i]=Math.round(clamp(r0*(1+.44*lift)+6*lift,0,255));
      out[i+1]=Math.round(clamp(g0*(1+.50*lift)+10*lift,0,255));
      out[i+2]=Math.round(clamp(b0*(1+.58*lift)+18*lift,0,255));
    }
  }
