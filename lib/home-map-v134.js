...
// Albedo remains locked. v133 uses a newly rebuilt scalar historical night signal
// sampled through the same approved geographic field as the map; colour and bloom stay runtime-only.
let textures;
...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    const signal=gunzipSync(readFileSync(asset('night-signal-v133.bin.gz')));
    if(signal.length!==WIDTH*HEIGHT*2)throw Error('Invalid v133 night signal');
    const cores=new Float32Array(WIDTH*HEIGHT);
    for(let p=0;p<cores.length;p++) {
      const energy=signal.readUInt16LE(p*2)/1024;
      // v133: preserve low/mid urban energy, then compress the high end with
      // one global soft-knee response. No regional or continent-specific gain.
      // The rebuilt scalar source already rejects the blue map backdrop, so
      // there is no terrain-luma gate that can erase dim lights on dark land.
      cores[p]=228*Math.pow(energy/(energy+8),.68);
    }
...
// v134 = locked 85% Option C + 15% Option D.
// Preserve v133's crisp city-network structure and hotspot compression; add only
// a restrained premium warm-gold finish through colour and halo tuning.
const POINT=[1,.77,.41],NEAR=[1,.69,.27],FAR=[1,.57,.17];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const local=near[p]*.47*NEAR[k]*night;
      const wide=far[p]*.25*FAR[k]*night;
