...
async function getTextures() {
  if(!textures) textures=Promise.all(['day.png','night.png'].map(name=>sharp(asset(name)).removeAlpha().raw().toBuffer()));
  return textures;
}
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let y=0;y<HEIGHT;y++) {
    ...
    for(let x=0;x<WIDTH;x++) {
      ...
      const mix=smooth(-7,4,elevation);
      const twilight=Math.exp(-(((elevation+2)/5)**2))*.8;
      // Restrained darker zone behind the native clock and agenda at lower left.
      const veil=1-.57*smooth(.30,.88,y/HEIGHT)*(1-smooth(.26,.59,x/WIDTH));
      const i=(y*WIDTH+x)*3;
      for(let k=0;k<3;k++) out[i+k]=clamp(Math.round((night[i+k]*(1-mix)+day[i+k]*mix+[4,14,26][k]*twilight)*veil),0,255);
    }
  }
  const overlays=[];
  ...
  if(p) {
    ...
    let label=await textImage(title,44,'#f4f6f8','Medium');
    ...
    const coords=await textImage(coord,32,'#afbac9');
    const boxWidth=Math.max(label.info.width,coords.info.width);
    let lx=p.x+40,ly=clamp(p.y-10,18,HEIGHT-100);
    if(lx+boxWidth>WIDTH-22) lx=p.x-40-boxWidth;
    ...
    overlays.push({input:Buffer.from(ring)});
    overlays.push({input:label.data,left:Math.round(lx),top:Math.round(ly)});
    overlays.push({input:coords.data,left:Math.round(lx),top:Math.round(ly+label.info.height+8)});
  }
  const border=Buffer.from(`<svg width="${WIDTH}" height="${HEIGHT}"><rect x="1.5" y="1.5" width="${WIDTH-3}" height="${HEIGHT-3}" rx="40" fill="none" stroke="#465767" stroke-width="3"/></svg>`);
