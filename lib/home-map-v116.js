...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let y=0;y<HEIGHT;y++) {
    ...
    for(let x=0;x<WIDTH;x++) {
      ...
      const i=(y*WIDTH+x)*3;
      // Midpoint between v114's night gain (1) and v115's (.32 to 1).
      // This restores dark land detail without lifting the brightest city lights.
      const nightGain=.66+.34*smooth(48,150,Math.max(night[i],night[i+1],night[i+2]));
