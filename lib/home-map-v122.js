...
async function getTextures() {
  if(!textures) textures=Promise.all([
    ...['day-v121.png','night-v121.png','lights-v121.png'].map(name=>sharp(asset(name)).removeAlpha().raw().toBuffer()),
    ...['land-v122.png','borders-v122.png'].map(name=>sharp(asset(name)).greyscale().raw().toBuffer()),
  ]);
  return textures;
}
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  const [day,night,lights,land,borders]=await getTextures();
  ...
  for(let y=0;y<HEIGHT;y++) {
    ...
    for(let x=0;x<WIDTH;x++) {
      ...
      const veil=1-.22*smooth(.38,.93,y/HEIGHT)*(1-smooth(.28,.59,x/WIDTH));
      const i=(y*WIDTH+x)*3;
      ...
      // Graphite terrain: reduce the flat grey floor, retain relief highlights,
      // and desaturate land independently of the ink-coloured oceans.
      const borderAlpha=borders[pixel]/255*(.52+.12*mix);
      for(let k=0;k<3;k++) {
        const dayTone=day[i+k]*(1-l)+l*Math.max(0,(dayLuma*.25+day[i+k]*.75-13)*1.12);
        const nightTone=night[i+k]*(1-l)+l*Math.max(0,(nightLuma*.22+night[i+k]*.78-6)*1.10);
        const terrain=nightTone*(1-mix)+dayTone*daylight*mix+atmosphere*SCATTER[k];
        // Geographic borders are independent of terrain texture and solar tint.
        // Compose before city lights so illuminated cities retain their cores.
        const lined=terrain*(1-borderAlpha)+[124,137,155][k]*borderAlpha;
        out[i+k]=clamp(Math.round(lined*veil*vignette+lights[i+k]*(1-mix)*vignette),0,255);
      }
    }
  }
  const overlays=[];
