...
export const WIDTH=1484, HEIGHT=640, NORTH=85, SOUTH=-65;
...
async function getTextures() {
  if(!textures) textures=Promise.all(['day-v114.png','night-v114.png'].map(name=>sharp(asset(name)).removeAlpha().raw().toBuffer()));
  return textures;
}
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let y=0;y<HEIGHT;y++) {
    ...
    for(let x=0;x<WIDTH;x++) {
      ...
      const daylight=1.10+.20*Math.sqrt(Math.max(0,Math.sin(elevation*RAD)));
      const nx=(x/WIDTH-.50)/.72,ny=(y/HEIGHT-.44)/.95;
      const vignette=1-.28*Math.min(1,nx*nx+ny*ny);
      // Restrained darker zone behind the native clock and agenda at lower left.
      const veil=1-.46*smooth(.38,.93,y/HEIGHT)*(1-smooth(.28,.59,x/WIDTH));
      const i=(y*WIDTH+x)*3;
      // One solar mask covers land and ocean. Preserve the brightest city lights
      // while darkening the night base; no added rim or decorative halo.
      const nightGain=.32+.68*smooth(48,150,Math.max(night[i],night[i+1],night[i+2]));
      for(let k=0;k<3;k++) out[i+k]=clamp(Math.round((night[i+k]*nightGain*(1-mix)+(day[i+k]*daylight+[4,8,12][k])*mix)*veil*vignette),0,255);
    }
  }
  const overlays=[];
  ...
  if(p) {
    ...
    let lx=p.x+56,ly=clamp(p.y-3,18,HEIGHT-100);
    if(lx+boxWidth>WIDTH-22) lx=p.x-56-boxWidth;
    lx=clamp(lx,20,WIDTH-boxWidth-20);
    // Move labels above the native agenda when the marker is at lower left.
    if(lx<770&&ly>248) ly=188;
    const ring=`<svg width="${WIDTH}" height="${HEIGHT}"><defs><radialGradient id="glow"><stop stop-color="#a8ff00" stop-opacity=".28"/><stop offset="1" stop-color="#a8ff00" stop-opacity="0"/></radialGradient></defs><circle cx="${p.x}" cy="${p.y}" r="48" fill="url(#glow)"/><circle cx="${p.x}" cy="${p.y}" r="25" fill="#09121c" fill-opacity=".7" stroke="#baff12" stroke-width="5"/><circle cx="${p.x}" cy="${p.y}" r="11" fill="#f7faf9"/></svg>`;
    const shadow=`<svg width="${WIDTH}" height="${HEIGHT}"><defs><filter id="soft"><feGaussianBlur stdDeviation="9"/></filter></defs><rect x="${lx-7}" y="${ly-6}" width="${boxWidth+14}" height="${label.info.height+(location.city?coords.info.height+8:0)+12}" rx="12" fill="#02070c" fill-opacity=".6" filter="url(#soft)"/></svg>`;
    ...
    if(location.city) overlays.push({input:coords.data,left:Math.round(lx),top:Math.round(ly+label.info.height+8)});
  }
  const border=Buffer.from(`<svg width="${WIDTH}" height="${HEIGHT}"><defs><linearGradient id="rim" x2=".7" y2="1"><stop stop-color="#64798b" stop-opacity=".85"/><stop offset=".35" stop-color="#33424e"/><stop offset="1" stop-color="#566977" stop-opacity=".7"/></linearGradient></defs><rect x="1.5" y="1.5" width="${WIDTH-3}" height="${HEIGHT-3}" rx="40" fill="none" stroke="url(#rim)" stroke-width="3"/></svg>`);
  overlays.push({input:border});
  const clip=Buffer.from(`<svg width="${WIDTH}" height="${HEIGHT}"><rect width="${WIDTH}" height="${HEIGHT}" rx="40" fill="white"/></svg>`);
