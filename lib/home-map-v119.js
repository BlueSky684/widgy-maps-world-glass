...
async function getTextures() {
  if(!textures) textures=Promise.all(['day-v119.png','night-v119.png'].map(name=>sharp(asset(name)).removeAlpha().raw().toBuffer()));
  return textures;
}
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let y=0;y<HEIGHT;y++) {
    ...
    for(let x=0;x<WIDTH;x++) {
      ...
      const daylight=1.04+.16*Math.sqrt(Math.max(0,Math.sin(elevation*RAD)));
      const nx=(x/WIDTH-.50)/.72,ny=(y/HEIGHT-.44)/.95;
      const vignette=1-.20*Math.min(1,nx*nx+ny*ny);
      ...
      for(let k=0;k<3;k++) out[i+k]=clamp(Math.round((night[i+k]*(1-mix)+day[i+k]*daylight*mix)*veil*vignette),0,255);
    }
  }
  const overlays=[];
