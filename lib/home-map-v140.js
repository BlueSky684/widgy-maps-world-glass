...
// v140 uses THREE independent scalar layers rebuilt from the untouched NASA source:
// urban network, city body and city core. No previous generated night signal is reused.
let textures;
...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    const networkRaw=gunzipSync(readFileSync(asset('night-network-v140.bin.gz')));
    const bodyRaw=gunzipSync(readFileSync(asset('night-body-v140.bin.gz')));
    const coreRaw=gunzipSync(readFileSync(asset('night-core-v140.bin.gz')));
    const expected=WIDTH*HEIGHT*2;
    if(networkRaw.length!==expected||bodyRaw.length!==expected||coreRaw.length!==expected)
      throw Error('Invalid v140 night layer');
    ...
    for(let p=0;p<networkTone.length;p++) {
      ...
      networkTone[p]=n<.08?0:168*Math.pow(n/(n+3.0),.70);
      bodyTone[p]=b<=0?0:236*Math.pow(b/(b+5.2),.64);
      coreTone[p]=c<=0?0:220*Math.pow(c/(c+13.0),.78);
    }
...
// v140 multi-layer luxury palette.
// Network is clean yellow-gold, city body is a slightly deeper warm gold,
// and core is restrained ivory/white-gold. No shine/starburst layer exists.
const NETWORK=[1,.90,.38],BODY=[1,.87,.26],IVORY=[1,.98,.72];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let p=0;p<darkness.length;p++) {
    ...
    networkSignal[p]=Math.round(Math.min(160,networkTone[p]*night));
    bodySignal[p]=Math.round(Math.min(205,bodyTone[p]*night));
    coreSignal[p]=Math.round(Math.min(172,coreTone[p]*night));
  }
  ...
  const [bodyNear,bodyWide]=await Promise.all([.82,1.82].map(sigma=>
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const network=networkSignal[p]*.40*NETWORK[k];
      const bodyPoint=bodySignal[p]*.24*BODY[k];
      const cityBody=bodyNear[p]*.55*BODY[k]*night;
      const cityHalo=bodyWide[p]*.085*BODY[k]*night;
      const ivoryCore=coreSignal[p]*.48*IVORY[k];
