...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    for(let p=0;p<networkTone.length;p++) {
      ...
      networkTone[p]=n<.10?0:148*Math.pow(n/(n+3.4),.74);
      bodyTone[p]=b<=0?0:220*Math.pow(b/(b+6.0),.68);
      coreTone[p]=c<=0?0:208*Math.pow(c/(c+14.5),.80);
    }
...
const NETWORK=[1,.88,.50],BODY=[1,.86,.36],IVORY=[1,.98,.86];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let p=0;p<darkness.length;p++) {
    ...
    networkSignal[p]=Math.round(Math.min(145,networkTone[p]*night));
    bodySignal[p]=Math.round(Math.min(190,bodyTone[p]*night));
  ...
  const [bodyNear,bodyWide]=await Promise.all([.78,1.72].map(sigma=>
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const network=networkSignal[p]*.35*NETWORK[k];
      const bodyPoint=bodySignal[p]*.18*BODY[k];
      const cityBody=bodyNear[p]*.44*BODY[k]*night;
      const cityHalo=bodyWide[p]*.065*BODY[k]*night;
      const ivoryCore=coreSignal[p]*.45*IVORY[k];
