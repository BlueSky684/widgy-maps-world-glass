...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    for(let p=0;p<networkTone.length;p++) {
      ...
      networkTone[p]=n<.10?0:148*Math.pow(n/(n+3.2),.72);
      bodyTone[p]=b<=0?0:178*Math.pow(b/(b+7.2),.74);
      coreTone[p]=c<=0?0:168*Math.pow(c/(c+16),.82);
    }
...
const NETWORK=[1,.88,.50],BODY=[1,.84,.39],IVORY=[1,.97,.84];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let p=0;p<darkness.length;p++) {
    ...
    networkSignal[p]=Math.round(Math.min(150,networkTone[p]*night));
    bodySignal[p]=Math.round(Math.min(170,bodyTone[p]*night));
    coreSignal[p]=Math.round(Math.min(150,coreTone[p]*night));
  }
  ...
  const [bodyNear,bodyWide]=await Promise.all([.72,1.65].map(sigma=>
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const network=networkSignal[p]*.58*NETWORK[k];
      const cityBody=bodyNear[p]*.34*BODY[k]*night;
      const cityHalo=bodyWide[p]*.055*BODY[k]*night;
      const ivoryCore=coreSignal[p]*.34*IVORY[k];
      const transmission=(1-network/255)*(1-cityBody/255)*(1-cityHalo/255)*(1-ivoryCore/255);
