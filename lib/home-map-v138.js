...
// Albedo remains locked. v137 uses a completely new approved HD scalar master rebuilt
// from the untouched NASA source. No previous generated night signal is consumed.
let textures;
...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    const signal=gunzipSync(readFileSync(asset('night-signal-v137.bin.gz')));
    if(signal.length!==WIDTH*HEIGHT*2)throw Error('Invalid v137 night signal');
    ...
    for(let p=0;p<cores.length;p++) {
      ...
      cores[p]=238*Math.pow(energy/(energy+6.9),.78);
    }
...
// v138 technical candidate: Option B + approved Yellow-Gold + ZERO STARBURST.
// The clean v137 scalar master is preserved. No shine/sparkle layer exists.
// Strong cores are globally soft-capped into compact round points; only a
// restrained isotropic micro-glow remains around the urban network.
const POINT=[1,.84,.57],NEAR=[1,.74,.34],FAR=[1,.63,.22];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let p=0;p<darkness.length;p++) {
    ...
    const compact=raw<=148?raw:148+54*(1-Math.exp(-(raw-148)/54));
    nightSignal[p]=Math.round(compact);
    // Glow gets a lower ceiling than the point core so dense cities stay compact.
    glowSignal[p]=Math.round(Math.min(compact,172));
  }
  const [near,far]=await Promise.all([.46,1.25].map(sigma=>
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const local=near[p]*.17*NEAR[k]*night;
      const wide=far[p]*.025*FAR[k]*night;
