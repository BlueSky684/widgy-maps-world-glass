...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    for(let p=0;p<cores.length;p++) {
      ...
      // Slightly lower shoulder than v130 protects bright centres while weak urban points survive.
      // This artistic response is not a calibrated radiance measurement.
      const response=228*Math.pow(1-Math.exp(-energy/28),.65);
...
// v131 keeps the v130 signal and terrain but warms and trims emission slightly.
// Point and halo responses remain independent; bloom is made from night-masked light.
const POINT=[1,.76,.40],NEAR=[1,.67,.25],FAR=[1,.56,.17];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const local=near[p]*.52*NEAR[k]*night;
      const wide=far[p]*.33*FAR[k]*night;
  ...
  if(p) {
    ...
    const ring=`<svg width="${WIDTH}" height="${HEIGHT}"><defs><radialGradient id="glow"><stop stop-color="#a8ff00" stop-opacity=".22"/><stop offset="1" stop-color="#a8ff00" stop-opacity="0"/></radialGradient></defs><circle cx="${p.x}" cy="${p.y}" r="${42*S}" fill="url(#glow)"/><circle cx="${p.x}" cy="${p.y}" r="${22*S}" fill="#09121c" fill-opacity=".7" stroke="#baff12" stroke-width="${4.5*S}"/><circle cx="${p.x}" cy="${p.y}" r="${9.5*S}" fill="#f7faf9"/></svg>`;
