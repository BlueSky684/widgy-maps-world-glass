...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let y=0;y<HEIGHT;y++) {
    ...
    for(let x=0;x<WIDTH;x++) {
      ...
      const mix=smooth(-8,7,elevation);
      // A wider, restrained atmospheric band follows the actual solar horizon.
      const twilight=Math.exp(-(((elevation+1)/10)**2));
      const daylight=.78+.22*Math.sqrt(Math.max(0,Math.sin(elevation*RAD)));
      ...
      const i=(y*WIDTH+x)*3;
      for(let k=0;k<3;k++) out[i+k]=clamp(Math.round((night[i+k]*(1-mix)+day[i+k]*mix*daylight+[6,21,38][k]*twilight)*veil*vignette),0,255);
    }
  }
  const overlays=[];
