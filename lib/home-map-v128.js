...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    const [glow,neighborhood]=await Promise.all([
      sharp(lights,raw).blur(2.4).raw().toBuffer(),
      sharp(lights,raw).blur(4.2).raw().toBuffer()
    ]);
    // Resolve point-like light detail separately from broad, continuous bands.
    // This is a display treatment of the illustrative source, not new city data.
    const emission=Buffer.alloc(lights.length);
    for(let i=0;i<lights.length;i+=3) {
      const peak=Math.max(lights[i],lights[i+1],lights[i+2]);
      if(!peak)continue;
      const surrounding=Math.max(neighborhood[i],neighborhood[i+1],neighborhood[i+2]);
      const detail=clamp((peak-surrounding)/peak,0,1);
      const energy=peak*(.28+.72*detail);
      // A smooth highlight shoulder keeps bright clusters from clipping white.
      const intensity=246*(1-Math.exp(-energy/110));
      const hue=[lights[i]/peak,Math.min(.89,lights[i+1]/peak),Math.min(.62,lights[i+2]/peak)];
      for(let k=0;k<3;k++)emission[i+k]=Math.round(intensity*hue[k]+glow[i+k]*.10);
    }
    ...
    return {base,emission,sinLat,cosLat,sinLon,cosLon};
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  const {base,emission,sinLat,cosLat,sinLon,cosLon}=await getTextures();
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    const gain=.62*night+(.92+.08*Math.max(0,dot))*day;
    ...
    for(let k=0;k<3;k++) {
      ...
      const city=emission[i+k]*night;
      const backdrop=clamp((terrain+air)*veil,0,255);
      out[i+k]=Math.round(255-(255-backdrop)*(1-city/255));
    }
  }
  const overlays=[];
