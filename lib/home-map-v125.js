...
async function getTextures() {
  if(!textures) textures=(async()=>{
    ...
    const interior=await sharp(land,{raw:{width:WIDTH,height:HEIGHT,channels:1}})
      .blur(3).raw().toBuffer();
    return {day,night,lights,land,borders,local,bloom,interior};
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  const {day,night,lights,land,borders,local,bloom,interior}=await getTextures();
  ...
  for(let y=0;y<HEIGHT;y++) {
    ...
    for(let x=0;x<WIDTH;x++) {
      ...
      const daylight=.98+.08*Math.sqrt(Math.max(0,Math.sin(elevation*RAD)));
      ...
      // Local-contrast blur crosses the coastline into black ocean; without
      // this interior mask it adds a pale outline instead of terrain detail.
      const coastDetail=.25+.75*smooth(.65,.97,interior[pixel]/255);
      const detail=(dayLuma-local[pixel])*1.1*coastDetail;
      const borderAlpha=borders[pixel]/255*(.29+.17*mix);
      for(let k=0;k<3;k++) {
        const dayTone=day[i+k]*(1-l)+l*Math.max(0,
          // Lower the grey midtone floor, retain cool colour and local detail.
          (dayLuma*.12+day[i+k]*.88-15)*1.03+detail);
        ...
        const borderColor=[121-12*mix,139-14*mix,161-16*mix][k];
        const lined=terrain*(1-borderAlpha)+borderColor*borderAlpha;
