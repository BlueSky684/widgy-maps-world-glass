...
// v137 approved reference target: clean premium YELLOW-GOLD, not rose/copper.
// Direct point energy carries the HD urban network; glow is narrow and restrained.
// White-gold brilliance is limited to the strongest cores to avoid a starfield look.
const POINT=[1,.84,.57],NEAR=[1,.74,.34],FAR=[1,.63,.22],SHINE=[1,.95,.76];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  const [near,far]=await Promise.all([.50,1.45].map(sigma=>
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const local=near[p]*.21*NEAR[k]*night;
      const wide=far[p]*.045*FAR[k]*night;
      const shine=Math.max(0,nightSignal[p]-152)*.13*SHINE[k];
      const transmission=(1-point/255)*(1-local/255)*(1-wide/255)*(1-shine/255);
