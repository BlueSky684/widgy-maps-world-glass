...
// v133 Night Lighting Rebuild: weak/mid lift + soft-knee hotspot compression.
// Point and halo responses remain separate so dense regions retain visible structure.
const POINT=[1,.76,.39],NEAR=[1,.66,.23],FAR=[1,.55,.15];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const local=near[p]*.43*NEAR[k]*night;
      const wide=far[p]*.24*FAR[k]*night;
