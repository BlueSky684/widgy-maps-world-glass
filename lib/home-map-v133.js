...
import {buildNightSignalV133} from './night-signal-v133.js';
...
const mesh=JSON.parse(readFileSync(asset('marker-mesh-v133.json'),'utf8'));
...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    const cores=await buildNightSignalV133({base,width:WIDTH,height:HEIGHT});
...
// v133 rebuilds the city-light signal independently from lights-v121.png.
// Terrain, the solar field and decorative blue atmosphere remain locked to the approved v129-v132 look.
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const point=nightSignal[p]*.92*POINT[k];
      const local=near[p]*.42*NEAR[k]*night;
      const wide=far[p]*.22*FAR[k]*night;
