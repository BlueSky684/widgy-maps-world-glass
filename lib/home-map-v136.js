...
// Albedo remains locked. v136 uses a new HD scalar master rebuilt directly from the original
// NASA source with 5x5 subpixel sampling. Colour, brilliance and glow remain runtime-only.
let textures;
...
async function getTextures() {
  if(!textures)textures=(async()=>{
    ...
    const signal=gunzipSync(readFileSync(asset('night-signal-v136.bin.gz')));
    if(signal.length!==WIDTH*HEIGHT*2)throw Error('Invalid v136 night signal');
    const cores=new Float32Array(WIDTH*HEIGHT);
    for(let p=0;p<cores.length;p++) {
      const energy=signal.readUInt16LE(p*2)/384;
      // v136 HD soft-knee: preserve weak network lines, keep clear separation in
      // dense urban regions and give strong cores a restrained premium lift.
      // One global response only; no regional or continent-specific gain.
      cores[p]=242*Math.pow(energy/(energy+6.6),.76);
    }
...
// v136 HD luxury target: sharp realistic city networks first, premium finish second.
// Direct champagne-gold point energy carries the structure. A tiny warm halo and
// highlight-only white-gold brilliance add luxury without turning lights into neon.
const POINT=[1,.82,.53],NEAR=[1,.70,.31],FAR=[1,.59,.20],SHINE=[1,.94,.78];
...
export async function renderHomeMap({date=new Date(),location=null}={}) {
  ...
  const [near,far]=await Promise.all([.52,1.55].map(sigma=>
  ...
  for(let y=0;y<HEIGHT;y++)for(let x=0;x<WIDTH;x++) {
    ...
    for(let k=0;k<3;k++) {
      ...
      const wide=far[p]*.055*FAR[k]*night;
      const shine=Math.max(0,nightSignal[p]-138)*.20*SHINE[k];
