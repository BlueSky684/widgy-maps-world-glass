...
## v127: rebuild from the approved reference

The user rejected further adjustments to the old construction and requested rebuilding from the approved image. The intermediate v126 glow experiment was therefore not released.

v127 uses **new artwork assets**, `reference-base-v127.png` and `reference-lights-v127.png`, reconstructed by separate built-in imagegen extraction edits of `approved-map-v123.png`. The first removes fixed lighting and city emission while retaining the reference layout, colour and border treatment; the second isolates the warm emission. These are illustrative images, not NASA satellite data, and extraction is not a guarantee of pixel-identical geometry or emission. Neither old terrain files nor the old land/border masks are loaded by the v127 renderer. Prompt summaries, source identity and hashes are recorded in `reference-provenance-v127.json`.

The artwork is kept in its original composition rather than warped to the old equirectangular map. `tools/register-reference-v127.py` uses NumPy/SciPy to calibrate 68 measured artwork landmarks with 122 non-folded affine triangles. Its coordinate field is encoded as row-wise horizontal delta signed little-endian int16 centidegrees, gzip compressed. The server decodes it and computes astronomical solar elevation per pixel. The same mesh positions the approximate IP marker. This is approximate illustrated-atlas calibration, **not survey-grade geography**. The faint straight grid is decorative rather than a labelled geographic graticule.

The blue horizon glow is generated from solar elevation. The reference-derived emission and its soft bloom are multiplied in full by the local night/twilight contribution, reaching zero in full daylight. There is no fixed Atlantic illumination arc. Updates occur on Widgy data refresh. The new native frame matches 1827 x 861; only map-layer geometry/URL and release metadata change. All other native layers, including the no-seconds 24-hour Live Timer, remain intact.

Build: `python3 tools/register-reference-v127.py`, then `node tools/build-widgy-v127.mjs`. Verify with `node tools/test-reference-map-v127.mjs`: landmark consistency, coordinate decoding, four-time city emission changes, seasonal polar shading, fresh server-time API routing, archived v125 and absence of old texture dependencies.

## v128: refine excessive night highlights

The v127 device screenshot showed broad white light bands and insufficiently visible night terrain. v128 retains the same artwork and geographic calibration. Its runtime display treatment separates local light detail from a 4.2-pixel neighborhood, attenuates broad emission relative to point detail, applies a smooth highlight shoulder with a warm colour limit, and retains a small soft bloom. These operations do not add real city measurements or remove every illustrated line in the source. The night terrain gain increases from 0.42 to 0.62; full daylight is unchanged. The complete emission remains multiplied by the dynamic night/twilight contribution.

Only native map layer 6170's URL/script and release metadata change from v127. All native layout, clock, location behavior, astronomical shading and refresh semantics are retained. Archived v127 remains unchanged. Build with `node tools/build-widgy-v128.mjs`; verify with `node tools/test-night-lighting-v128.mjs`, including actual rendered daylight preservation, moving night lights and the reduction of clipped near-white pixels.
