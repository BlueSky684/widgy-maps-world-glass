...
function blend(p,x,y,r,g,b,a){
  ...
  const i=(p.width*y+x)<<2,sa=clamp(a,0,255)/255,da=p.data[i+3]/255,oa=sa+da*(1-sa);
  if(oa<=0)return;
...
export default async function handler(req,res){
 try{
  ...
  circle(png,x,y,34,blue,30);circle(png,x,y,23,blue,78);
  circle(png,x,y,13,blue,255);circle(png,x,y,6,[255,255,255],255);

  console.log('WIDGY_MARKER_GEO',JSON.stringify({lat,lon,x:Math.round(x*10)/10,y:Math.round(y*10)/10,city:req.headers['x-vercel-ip-city']||null,country:req.headers['x-vercel-ip-country']||null}));
  const buffer=PNG.sync.write(png);
  res.setHeader('Content-Type','image/png');res.setHeader('Cache-Control','no-store, max-age=0');res.status(200).send(buffer);
 }catch(err){res.status(500).json({error:err?.message||'marker failed'});}
}
