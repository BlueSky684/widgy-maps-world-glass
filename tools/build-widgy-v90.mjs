...
const before=JSON.parse(read('widgy-v89.json')),data=structuredClone(before);
...
const maxFill=.04;
// Focus the tiny addition closer to the rightmost edge of the upper cap.
// Its sin-fourth taper has zero first, second and third derivatives at
// both boundaries; the crown, lower cap and straight-side join are fixed.
for(let n=17;n<22;n++){
 const t=(n-16)/6;
...
item.id='E785CA90-9F64-4DB1-A105-528838F93C90';
item.name='Right Edge Rain Fill v90';
layer['3']=item.id;layer['2']=Buffer.from(JSON.stringify(library)).toString('base64');
layer.s=layer.s.replace('Tiny Upper Right Fill · v89','Right Edge Fill · v90');
...
data['3']='Native Weather Icons Master Board v90 - Right Edge Rain Fill';
data['4']='Based on v89 and feedback IMG_9364. Add at most 0.04 reference pixel horizontally nearer the rightmost edge of the upper cap of the leftmost light-rain stroke. A smooth sin-fourth taper changes only points 17 through 21. Every other contour point remains identical to v89. The outline stays convex and contains the previous outline, within the same body width. The crown, side join, lower cap, frame and other layers are unchanged. Native appearance requires on-device review.';
...
write('widgy-v90.json',payload);
const html=read('widgy-v89.html').replaceAll('v89','v90')
 .replace('תוספת עדינה לקשת העליונה — פס בדיקה אחד','דיוק הקצה המעוגל — פס בדיקה אחד')
